...
function install()
{
    ...
    echo "******************************************************************************"
    echo -e "\033[42;34m ==============开始检查基础安装包......================= \033[0m"
    ...
    if [ -f /opt/soft/analysys_installer_base_$osType.tar.gz ];then
        ...
        if [ -f /opt/soft/analysys_installer_base_$osType.tar.gz.md5 ];then
            ...
            echo "缺少md5检验文件analysys_installer_base_$osType.tar.gz.md5，请下载并放到/opt/soft/目录下。"
            exit 1
        fi

    else
        echo "没有检查到安装包，请将安装包analysys_installer_base_$osType.tar.gz和其对应的analysys_installer_base_$osType.tar.gz.md5文件一起放到/opt/soft/目录下"
        exit 1
...
if [ "${os_ver1}" != "${os_ver2}" ];then
	...
	echo -e "${c_red}当前系统的版本不适配当前的argo软件${c_end}"
	exit 98
fi
...
do
        #read -p "`echo -e "please enter the name of Ethernet，default [${c_yellow}eth0${c_end}]: "`" eth
        read -p "`echo -e "请输入SDK上报数据时访问该机器的IP(如果机器有外网ip，请输入外网ip；如果机器没有外网ip请输入内网ip): "`" IP_out
